...
// resourceDescription is the description GCP resources created by CDP carry;
// GCP-specific and never overridden.
const resourceDescription = "Generated by CDP."
...
// Register wires the GCP provider into the cloud-provider registry with cfg
// injected. main calls it once after building the config; client init is
// deferred to first use and memoized.
func Register(cfg *config.Config) {
	...
	p := &gcpProvider{cfg: cfg}
	var once sync.Once
	cfg.CloudProviders[types.GCP] = func() types.CloudProvider {
		once.Do(func() {
			...
			if err := p.init(projectID, computeClient, iamClient, sqlClient, storageClient); err != nil {
...
func initClients() (computeClient *http.Client, iamClient *http.Client, sqlClient *http.Client, storageClient *storage.Client, err error) {
...
func (p *gcpProvider) init(projectID string, computeHTTPClient *http.Client, iamHTTPClient *http.Client,
	sqlHTTPClient *http.Client, storageHTTPClient *storage.Client) error {
...
func (p gcpProvider) GetInstances() ([]*types.Instance, error) {
	...
	return getInstances(p.computeClient.Instances.AggregatedList(p.projectID), p.cfg.OwnerLabel)
}
...
func (p gcpProvider) GetStacks() ([]*types.Stack, error) {
	...
	for _, items := range instanceResponse.Items {
		for _, gcpInstance := range items.Instances {
			instance := newInstance(gcpInstance, p.cfg.OwnerLabel)
	...
	for instanceName, instance := range instancesByName {
		stackId, ok := instance.Tags[strings.ToLower(p.cfg.ResourceGroupingLabel)]
		if !ok {
			log.Warnf("[GCP] Skipping instance %s as it does not have %s tag", instance.Name, strings.ToLower(p.cfg.ResourceGroupingLabel))
			continue
		}
		...
		if _, ok := gcpStacks[stackId]; !ok {
			...
			for _, database := range databases {
				if stackId == database.Tags[strings.ToLower(p.cfg.ResourceGroupingLabel)] {
...
func (p gcpProvider) getNetworksBySelfLink() (map[string]*compute.Network, error) {
	...
	if resourceDescription != "" {
		descriptionFilter := fmt.Sprintf("description = \"%s\"", resourceDescription)
...
func (p gcpProvider) DeleteImages(images *types.ImageContainer) []error {
	...
	return deleteImages(getAggregator, images.Get(types.GCP), p.cfg.DryRun)
}
...
func getInstances(aggregator instancesListAggregator, ownerLabel string) ([]*types.Instance, error) {
	...
	for _, items := range instanceList.Items {
		for _, inst := range items.Instances {
			instances = append(instances, newInstance(inst, ownerLabel))
		}
	}
...
func deleteImages(getAggregator func(string) imageDeleteAggregator, images []*types.Image, dryRun bool) []error {
	wg := sync.WaitGroup{}
	wg.Add(len(images))
	errChan := make(chan error)
...
func newInstance(inst *compute.Instance, ownerLabel string) *types.Instance {
	...
	for _, items := range diskList.Items {
		for _, gDisk := range items.Disks {
			...
			aDisk := &types.Disk{
				...
				Owner:     gDisk.Labels[p.cfg.OwnerLabel],
...
func (p gcpProvider) GetStorages() ([]*types.Storage, error) {
	...
	bucketIt := p.storageClient.Buckets(context.Background(), p.projectID)
	storages := []*types.Storage{}
	var err error = nil
	for {
		bucketAttrib, err := bucketIt.Next()
		if err == iterator.Done {
			break
		} else if err != nil {
			// TODO: Handle error.
			log.Errorf("[GCP] failed to retrieve bucket items error: %v", err)
			break
		}
		storage := &types.Storage{
			Name:      bucketAttrib.Name,
			Owner:     bucketAttrib.Labels["owner"],
			Created:   bucketAttrib.Created,
			CloudType: types.GCP,
			Tags:      bucketAttrib.Labels,
			Region:    bucketAttrib.Location,
		}
		storages = append(storages, storage)
		log.Debugf("[GCP] Storage: %v, CreationTimeStamp: %s", storage, bucketAttrib.Created)
	}
	return storages, err
}
...
func (p gcpProvider) CleanupStorages(storageContainer *types.StorageContainer, retentionDays int) []error {
	...
	for r, b := range bucketsByRegion {
		go func(storageClient storage.Client, region string, bucketsInRegion []*types.Storage) {
			defer wg.Done()
			...
			for i := 0; i < len(bucketsInRegion); i += 1 {
				...
				objectsIterator := bucketHandle.Objects(ctxbg, nil)
				var objectAttribsList []*storage.ObjectAttrs
				for {
					objectAttribs, err := objectsIterator.Next()
					if err == iterator.Done {
						break
					} else if err != nil {
					...
					if objectAttribs != nil {
						objectAttribsList = append(objectAttribsList, objectAttribs)
					}

					log.Debugf("Bucket name: %s - Page: %d", bucketName, pageCounter)
					pageCounter++
				}

				log.Debugf("[GCP] Total number of collected objects: %d", len(objectAttribsList))
				for _, objectAttrib := range objectAttribsList {
					if objectAttrib.Updated.Before(retentionTime) {
						log.Infof("[GCP] File '%s' in S3 bucket will be deleted because it is older than %s.", objectAttrib.Name, retentionTime)
						if p.cfg.DryRun {
							cleanedUpStorage += int64(objectAttrib.Size)
							continue
						}
						err := storageClient.Bucket(objectAttrib.Bucket).Object(objectAttrib.Name).Delete(ctxbg)
						if err != nil {
							errChan <- err
						} else {
							cleanedUpStorage += int64(objectAttrib.Size)
				...
				if bucketsInRegion[i].Created.Before(retentionTime) {
					...
					if !p.cfg.DryRun {
						err := storageClient.Bucket(bucketName).Delete(context.Background())
						if err != nil {
							errChan <- err
						}
					}
